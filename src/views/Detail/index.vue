            ...
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  :key="item2.id"
                  @click="changActive(item2, item.spuSaleAttrValueList)"
                >
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getGoods", this.$route.params.skuid);
  },
  computed: {
  ...
  methods: {
    changActive(saleAttrValues, arr) {
      arr.forEach((item) => {
        item.isChecked = "0";
      });
      saleAttrValues.isChecked = 1;
    },
    changeSkuNum(e) {
    ...
    async addCurt(skuNum) {
     try {
        await this.$store.dispatch("getGoodsCurt", {
